...
RUST_TEST = f"cd {APP} && cargo llvm-cov --locked --fail-under-lines {RUST_COVERAGE_MINIMUM}"
...
def lockfile(names: list[str]) -> str:
    """The workspace's `Cargo.lock`, for a skeleton that depends on nothing: each member at `0.1.0`.

    Cargo writes exactly this for a dependency-free workspace — members sorted by name — so it is computed
    rather than committed, since the names are the project's. `--locked` everywhere in the gate then refuses
    any drift, the way it will once a dependency set is committed per variant.
    """
...
def repository_files(
    ...
    files["Cargo.lock"] = lockfile([crate_name(project_name, service) for service in services])
...
for app in {apps}; do
  (cd "$app" && cargo fmt --check && cargo clippy --locked --all-targets -- -D warnings \\
    && cargo llvm-cov --locked --fail-under-lines {RUST_COVERAGE_MINIMUM})
done
"""
    return files
